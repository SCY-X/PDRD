...
class ResNet(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], last_stride=2, projection=512, num_classes=1000, frozen_stages=-1):
        self.inplanes = 64
        super().__init__()
        ...
        self.fc = ClassBlock(512 * block.expansion, num_classes, num_bottleneck=projection)
...
def resnet18(pretrained=False, pretrained_path="", **kwargs):
    ...
    """
    model = ResNet(block=BasicBlock, layers=[2, 2, 2, 2], last_stride=kwargs["last_stride"], projection=kwargs["projection"], num_classes=kwargs["num_classes"])
    if pretrained:
        if pretrained_path != "":
...
def resnet34(pretrained=False, pretrained_path="", **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(block=BasicBlock, layers=[3, 4, 6, 3], last_stride=kwargs["last_stride"], projection=kwargs["projection"], num_classes=kwargs["num_classes"])
    if pretrained:
        if pretrained_path != "":
            pretrained_dict = {k.replace('module.', ""): v for k, v in torch.load(pretrained_path, weights_only=True).items()}
        else:
...
def resnet50(pretrained=False, pretrained_path="", **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(block=Bottleneck, layers=[3, 4, 6, 3], last_stride=kwargs["last_stride"], projection=kwargs["projection"], num_classes=kwargs["num_classes"])
    if pretrained:
        if pretrained_path != "":
            pretrained_dict = {k.replace('module.', ""): v for k, v in torch.load(pretrained_path, weights_only=True).items()}
        else:
...
def resnet101(pretrained=False, pretrained_path="", **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(block=Bottleneck, layers=[3, 4, 23, 3], last_stride=kwargs["last_stride"], projection=kwargs["projection"], num_classes=kwargs["num_classes"])
    if pretrained:
        if pretrained_path != "":
            pretrained_dict = {k.replace('module.', ""): v for k, v in torch.load(pretrained_path, weights_only=True).items()}
        else:
...
def resnet152(pretrained=False, pretrained_path="", **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(block=Bottleneck, layers=[3, 8, 36, 3], last_stride=kwargs["last_stride"], projection=kwargs["projection"], num_classes=kwargs["num_classes"])
    if pretrained:
        if pretrained_path != "":
            pretrained_dict = {k.replace('module.', ""): v for k, v in torch.load(pretrained_path, weights_only=True).items()}
        else:
