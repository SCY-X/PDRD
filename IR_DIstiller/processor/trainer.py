...
class KDTrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch):
        ...
        for n_iter, ((img, target), (kd_img, kd_target)) in enumerate(zip(self.student_train_loader, self.distillation_loader)):
            self.optimizer.zero_grad()

            img, target = img.to(self.device), target.to(self.device)
            kd_img, kd_target = kd_img.to(self.device), kd_target.to(self.device)

            if self.cuda_amp:
                with autocast(device_type="cuda"):
                    preds, losses_dict = self.distiller(image=img, kd_image=kd_img, target=target, kd_target=kd_target)
