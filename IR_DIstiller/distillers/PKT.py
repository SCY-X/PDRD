...
class PKT(Distiller):
    ...
    def forward_train(self, image, kd_image, target, kd_target, **kwargs):
        ...
        kd_loss = self.kd_loss_weight * pkt_loss(F.adaptive_avg_pool2d(kd_feature_student["feats"][-1], 1).flatten(1), F.adaptive_avg_pool2d(kd_feature_teacher["feats"][-1], 1).flatten(1))
