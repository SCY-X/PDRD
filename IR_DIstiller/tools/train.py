...
def main(cfg, student_train_loader, distillation_loader, num_classes, query_loader, gallery_loader):
    ...
    if cfg.DISTILLER.TYPE == "NONE":
        model_student = model_dict[cfg.DISTILLER.STUDENT_NAME](pretrained=True, pretrained_path=cfg.DISTILLER.STUDENT_PRETRAIN_PATH, 
                                                               last_stride=cfg.DISTILLER.STUDENT_LAST_STRIDE, projection = cfg.DISTILLER.STUDENT_PROJECTION_DIM, num_classes=num_classes)
        
        distiller = distiller_dict[cfg.DISTILLER.TYPE](model_student, cfg)

    else:
        model_teacher = model_dict[cfg.DISTILLER.TEACHER_NAME](pretrained=False, last_stride=cfg.DISTILLER.TEACHER_LAST_STRIDE, projection = cfg.DISTILLER.TEACHER_PROJECTION_DIM, num_classes=num_classes)
    ...
    if torch.cuda.device_count() > 1:
        base_params = distiller.module.get_train_base_parameters()
        extra_params = distiller.module.get_train_extra_parameters()
        base_flops = distiller.module.get_base_flops()
    else:
        base_params = distiller.get_train_base_parameters()
        extra_params = distiller.get_train_extra_parameters()
        base_flops = distiller.get_base_flops(cfg.INPUT.SIZE_TRAIN)
    ...
    with open('%s/speed.txt' % output_dir, 'a') as test_file:
        test_file.write('Training Parameters and FLOPs: \n')
