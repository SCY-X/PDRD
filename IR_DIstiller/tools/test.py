...
import sys
sys.path.append(os.path.abspath("/XXX/IR_Distiller"))
...
if __name__ == "__main__":
    ...
    distiller = distiller_dict["NONE"](model_student, cfg)

    
    distiller.load_state_dict(torch.load(os.path.join(output_dir, "student_{}.pth".format(cfg.TEST.WEIGHT)), weights_only=True))
    ...
    base_params = distiller.get_inference_base_parameters()
    base_flops = distiller.get_base_flops(cfg.INPUT.SIZE_TRAIN)

    print("The student base parameters: {:.3f} M and base flops: {:<8}\033[0m".format(
        base_params, base_flops))
    
    output_dir = os.path.join(cfg.OUTPUT_DIR.ROOT_PATH, cfg.OUTPUT_DIR.EXPERIMENT_NAME)
    with open('%s/speed.txt' % output_dir, 'a') as test_file:
        test_file.write('Inference Parameters and FLOPs: \n')
