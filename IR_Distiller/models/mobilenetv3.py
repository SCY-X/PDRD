...
class MobileNetV3(nn.Module):
    def __init__(self, last_stride = 2, projection=512, num_classes= 1000, mode="small", pretrained=True):
        ...
        self.fc = ClassBlock(self.out_channels, num_classes, num_bottleneck=projection)
...
def mobilenetv3_small(pretrained=False, pretrained_path="", **kwargs):
    ...
    if pretrained:
        model = MobileNetV3(last_stride=kwargs["last_stride"] , projection=kwargs["projection"], num_classes=kwargs["num_classes"],  mode="small", pretrained=True)

    else:
        model = MobileNetV3(last_stride=kwargs["last_stride"] , projection=kwargs["projection"], num_classes=kwargs["num_classes"], mode="small", pretrained=False)
